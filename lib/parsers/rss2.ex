defmodule Feedex.Parsers.RSS2 do
  alias Feedex.Helpers.Xml

  def valid?(xml) do
    Xml.first(xml, "/rss/channel") != nil
  end

  def parse(xml) do
    channel = Xml.first(xml, "/rss/channel")
    feed = %{
      ...
      updated:     Xml.first(channel, "pubDate")     |> Xml.date,
  ...
  defp extract_date(xml) do
    xml |> Xml.first("pubDate") |> Xml.date
  end
end
